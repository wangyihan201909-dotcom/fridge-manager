/*
 * CustomLcdDisplay —— 4.2" 墨水屏的 LVGL 显示实现。
 *
 * 移植自社区项目 cattei/xiaozhi-zectrix（zectrix 分支），
 * 该项目是 78/xiaozhi-esp32 的 fork，两者均为 MIT 授权：
 *
 *   Copyright (c) 2025 Shenzhen Xinzhi Future Technology Co., Ltd.
 *   SPDX-License-Identifier: MIT
 *
 * 相对上游的改动（本项目）：
 *   1. include "common/sleep_manager.h" 改为同目录的本地垫片 ——
 *      那个头文件在社区版仓库里并不存在；本项目一直开唤醒词、不深睡，
 *      垫片是空实现。**改回电池深睡时必须换成真的**，否则墨水屏会
 *      刷到一半断电留下残影。
 *   2. 新增 SetRawMode(bool) / raw_mode()：开启后 lvgl_flush_cb 直接
 *      lv_display_flush_ready 并返回，不往帧缓冲落笔，屏幕整块交给
 *      WriteRaw1bpp 灌云端渲染的位图。两套 UI 写同一个缓冲，
 *      不拦住就是谁后写谁赢。
 */
...
#include "sleep_manager.h"   /* 本地垫片：社区版仓库缺这个文件，见文件内注释 */
...
void CustomLcdDisplay::lvgl_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *color_p) {
    ...
    /* 冰箱页期间不让 LVGL 落笔。**flush_ready 仍然要调** ——
     * 不调的话 LVGL 会一直等这一帧渲染完成，整个 UI 线程卡死。 */
    if (driver->raw_mode_) {
...
void CustomLcdDisplay::SetRawMode(bool on) {
    if (raw_mode_ == on) return;
    raw_mode_ = on;
    ESP_LOGI(TAG, "原始模式 -> %s", on ? "开（屏幕归冰箱页）" : "关（交回小智）");
    if (!on) {
