...
public:
    ...
    /* 原始模式：开着时 LVGL 的 flush 回调直接返回，不碰 buffer，
     * 屏幕完全由 WriteRaw1bpp 决定（冰箱管家的云端位图）。
     * 关掉时会让 LVGL 整屏重绘一次 —— 它不知道屏幕被我们改过，
     * 只重画「它认为脏了」的区域的话会残留我们的位图。 */
    void SetRawMode(bool on);
    bool raw_mode() const { return raw_mode_; }
